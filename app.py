...
def predict():
    ...
    return render_template('linearRegression/linealRApplication.html',
                           columns=columns, prediction=prediction,
                           x_col=x_col, input_value=input_value)
...
def predict_logistic():
    ...
    result_data = train_predict_logistic(df_logistic, pace_input, shooting_input,
                                         passing_input, dribbling_input)
...
def perceptronApp():
    ...
    if request.method == 'POST':
        prediction = prediction_perceptron(data["model"], data["scaler"],
                                           data["X"], request.form)
